...
namespace check_services
{
    public class Program
    {
        ...
        private static int Main(string[] args)
        {
            ...
            // Handle Arguments
            returncode = Settings.HandleArguments(returncode, temp_excluded_services, temp_included_services, temp_stopped_services, temp_running_services, temp_categories, temp_warn_categories, temp_services_in_system_category,
            ...
            {
                temp = Inventory.OutputReadable(inventory_level, do_all_running_only, do_hide_empty_vars);
                return (int)ServiceState.ServiceUnknown;
            }
            else if (do_inventory == true && inventory_format == "csv")
            {
                temp = Inventory.OutputCSV(inventory_level, do_all_running_only);
                return (int)ServiceState.ServiceUnknown;
            }
            ...
            {
                temp = Inventory.OutputJSON(inventory_level, do_all_running_only);
                return (int)ServiceState.ServiceUnknown;
            }
            ...
            if (do_services == true)
            {
                returncode = Check.Services(inventory_level, returncode, do_all_running_only, do_all_starttypes, delayed_grace_duration, do_hide_category_from_output, do_single_check, expected_state);
            }
        ...
        private static int HandleExitText(int returncode, bool do_hide_long_output)
        {
            ...
            {
                int x = 1;
                foreach (string outputS in Check.listServiceOutput)
                {
                    if (x < Check.listServiceOutput.Count)
                    {
            ...
            string perfdata = "";
            foreach (string outputP in Check.listPerfData)
            {
